...
mem_init(void)
{
	...
	// Your code goes here:
	uint32_t i, npages_end;
	npages_end = ROUNDUP(npages*sizeof(struct PageInfo), PGSIZE);
	for (i = 0; i < npages_end; i += PGSIZE){
		page_insert(kern_pgdir, pa2page(PADDR((char *)pages + i)), (void*)(UPAGES+i), PTE_U|PTE_P); 
	}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(!(pgdir[PDX(va)] & PTE_P)){
		if (create == false) return NULL;
			...
			struct PageInfo  *pgtab_page = page_alloc(ALLOC_ZERO);
			if (pgtab_page == NULL)  return NULL;
			else{
				pgtab_page->pp_ref += 1;
				pgdir[PDX(va)] = page2pa(pgtab_page)|PTE_P|PTE_W;
			}
		}
	}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	// Fill this function in
	uint32_t inc = 0;
	for(inc; inc<size; inc+=PGSIZE){
		pte_t * pte = pgdir_walk(pgdir, (void *)(va+inc), true) ;
		*pte = (pa+inc) | perm | PTE_P; 
	}
}
